@if (isset($_COOKIE["study-case-category"]))
    @php $cookie_category = json_decode(stripslashes($_COOKIE["study-case-category"])) @endphp
@endif
@if (isset($_COOKIE["study-case-year"]))
    @php $cookie_year = json_decode(stripslashes($_COOKIE["study-case-year"])) @endphp
@endif
@if (isset($_COOKIE["study-case-month"]))
    @php $cookie_month = json_decode(stripslashes($_COOKIE["study-case-month"])) @endphp
@endif
...
<div id="wrapper-blog-filter" class="d-flex flex-column flex-lg-row align-items-lg-center justify-content-lg-between">
    <strong id="title" class="mb-4 mb-lg-0">
        @if (isset($_COOKIE["study-case-category"]) && $cookie_category != 'all')
            @php $term = get_term_by('id', $cookie_category, 'study-case-category'); @endphp
            {!! $term->name !!}
        @else
            Les dernières analyses
        @endif
    </strong>
    <form id="study-case-filter">
        ...
        @if (isset($_COOKIE["study-case-category"][0]) OR isset($_COOKIE["study-case-year"][0]) OR isset($_COOKIE["study-case-month"][0]) )
            @if (json_decode(stripslashes($_COOKIE["study-case-category"])) != 'all' OR json_decode(stripslashes($_COOKIE["study-case-year"])) != 'all' OR json_decode(stripslashes($_COOKIE["study-case-month"])) != 'all')
